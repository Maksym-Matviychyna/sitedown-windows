...
public sealed record SettingsKeyResult
{
...
public sealed class SettingsKeyService
{
    private readonly HttpClient _httpClient;

    // Same Settings Key config endpoint as the Android app.
    private static readonly Uri[] SettingsEndpoints =
    {
        new("https://sitedown.app/api/config/")
    };

    public SettingsKeyService()
    {
    ...
    public async Task<SettingsKeyResult> ApplySettingsKeyAsync(string settingsKey, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(settingsKey))
        {
            return new SettingsKeyResult { Success = false, Message = "Settings Key is empty." };
        }

        Exception? lastException = null;
        string lastResponse = string.Empty;

        foreach (var endpoint in SettingsEndpoints)
        {
            try
            {
                var requestJson = JsonSerializer.Serialize(new
                {
                    settings_key = settingsKey.Trim()
                });
                ...
                var parsed = ParseSettingsKeyResponse(lastResponse);
                if (parsed.Success)
                {
                    return parsed with { Message = string.IsNullOrWhiteSpace(parsed.Message) ? $"Settings Key accepted using {endpoint}" : parsed.Message };
                }
        ...
        {
            return new SettingsKeyResult { Success = false, Message = lastException.Message };
        }

        return new SettingsKeyResult
        {
            Success = false,
            Message = string.IsNullOrWhiteSpace(lastResponse)
                ? "Could not apply the Settings Key. Check the endpoint path on your website."
                : "Settings Key failed. Server response: " + TrimForLog(lastResponse)
        };
    }

    private static SettingsKeyResult ParseSettingsKeyResponse(string json)
    {
        if (string.IsNullOrWhiteSpace(json))
        {
            return new SettingsKeyResult { Success = false, Message = "Empty response from server." };
        }
        ...
        catch (JsonException)
        {
            ...
            {
                return new SettingsKeyResult { Success = true, Message = "Settings Key accepted successfully." };
            }
