...
{
    private const int CURRENT_VERSION_CODE = 7;
    private const string CURRENT_VERSION = "1.0.7";
    ...
    private readonly SettingsKeyService _settingsKeyService = new();
    ...
    private async void MainWindow_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        // Settings Keys are one-time-use. Never restore a previously entered Settings Key into the input field.
        SettingsKeyTextBox.Text = string.Empty;
        ReplaceSites(settings.Sites);
        SettingsKeyBlock.Visibility = _sites.Count > 0 ? Visibility.Collapsed : Visibility.Visible;

        // Re-save settings without any Settings Key value. This also cleans old settings files from earlier app versions.
        ...
        {
            StatusTextBlock.Text = "No Settings Key";
            _logger.Info("Enter a Settings Key to connect this Windows device.");
        }
    ...
    private async void ApplySettingsKeyButton_Click(object sender, RoutedEventArgs e)
    {
        await ApplySettingsKeyAndLoadSettingsAsync(startAfterSuccess: true);
    }
    ...
    private void StartButton_Click(object sender, RoutedEventArgs e)
    {
        if (_sites.Count == 0)
        {
            _logger.Info("No websites loaded yet. Apply the Settings Key first.");
            return;
        }
    ...
    private void StopButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        StatusTextBlock.Text = _sites.Count > 0 ? "Stopped" : "No Settings Key";
    ...
    {
        HideHelpScreen();
        SettingsKeyBlock.Visibility = SettingsKeyBlock.Visibility == Visibility.Visible
    ...
    private void SettingsKeyTextBox_TextChanged(object sender, System.Windows.Controls.TextChangedEventArgs e)
    {
        if (SettingsKeyPlaceholderTextBlock == null)
        {
            return;
        }

        SettingsKeyPlaceholderTextBlock.Visibility = string.IsNullOrWhiteSpace(SettingsKeyTextBox.Text)
            ? Visibility.Visible
            : Visibility.Collapsed;
    }

    private void SettingsKeySetupLink_RequestNavigate(object sender, System.Windows.Navigation.RequestNavigateEventArgs e)
    {
    ...
    private async Task ApplySettingsKeyAndLoadSettingsAsync(bool startAfterSuccess)
    {
        var settingsKey = SettingsKeyTextBox.Text.Trim();
        if (string.IsNullOrWhiteSpace(settingsKey))
        {
            _logger.Error("Settings Key is empty.");
            return;
        }

        ApplySettingsKeyButton.IsEnabled = false;
        StatusTextBlock.Text = "Saving Settings Key...";
        _logger.Info("Saving Settings Key and loading settings from SiteDown...");

        try
        {
            var wasRunning = _monitorService.IsRunning;
            var result = await _settingsKeyService.ApplySettingsKeyAsync(settingsKey);
            if (!result.Success)
            {
                StatusTextBlock.Text = _sites.Count > 0 ? "Stopped" : "No Settings Key";
            ...
            // Settings Keys are one-time-use. Clear the input immediately after the Settings Key is applied.
            SettingsKeyTextBox.Text = string.Empty;
            SettingsKeyBlock.Visibility = _sites.Count > 0 ? Visibility.Collapsed : Visibility.Visible;
            ...
            {
                StatusTextBlock.Text = "No Settings Key";
                _logger.Info("Settings Key was applied, but the server returned no websites yet. Create a new Settings Key when settings are ready.");
            }
        ...
        {
            ApplySettingsKeyButton.IsEnabled = true;
        }
    }
